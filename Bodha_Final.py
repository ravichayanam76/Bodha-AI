...
if st.session_state.role == "Examiner":
    if not st.session_state.is_authenticated:
        ...
        if st.sidebar.button("🔄 Clear All Results & Exams"):
        ...
        if uploaded_file and st.button("Generate & Publish Exam"):
            ...
            if quiz_data:
                ...
                st.success("✅ Exam published globally!")

        # --- EXAMINER: SEE STUDENT RESULTS ---
        st.write("---")
...
# --- STUDENT VIEW ---
    ...
    if not current_quiz:
        ...
        st.success("✅ You have already submitted your answers. Multiple attempts are not allowed.")
    else:
        student_name = st.text_input("Enter Your Full Name:", placeholder="Required to save score")
        ...
        if not submitted:
            ...
            rem = max(0, (exam_duration_min * 60) - elapsed)
        ...
        if submitted:
            if not student_name:
                ...
                st.metric("Your Result", f"{(score/len(current_quiz))*100:.1f}%", f"{score}/{len(current_quiz)}")
                st.download_button("📊 Download Result Report", data=result_report, file_name="my_results.txt")
                st.rerun()
